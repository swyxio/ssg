		...
		try {
			...
			watcher.on('build', (event: BuildEvent) => {
				if (event.errors.length) {
					...
					event.errors.filter((e: TODO_ANY) => !e.duplicate).forEach((error: TODO_ANY) => {
					...
					event.warnings.filter(e => !e.duplicate).forEach((warning: TODO_ANY) => {
...
prog.command('build [dest]')
	...
	.action(async (dest: string = '__sapper__/build', opts: {
		port: string,
		legacy: boolean,
		bundler?: 'rollup' | 'webpack',
		cwd: string,
...
prog.command('export [dest]')
	...
	.action(async (dest: string = '__sapper__/export', opts: {
		build: boolean,
		legacy: boolean,
		bundler?: 'rollup' | 'webpack',
		basepath?: string,
