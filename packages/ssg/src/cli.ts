  ...
  .action(
    async (opts: {
      ...
      try {
         ...
         *
         */
        const { getSSGDataOnce, watchSSGFiles, readSSGConfig } = await import(
          './cli-ssg'
        );
        ...
        watcher.on('invalid', (event: InvalidEvent) => {
          ...
          const changed = event.changed
            .map((filename: string) => path.relative(process.cwd(), filename))
            .join(', ');
          console.log(
        ...
        watcher.on('build', (event: BuildEvent) => {
          if (event.errors.length) {
            ...
            const hidden = event.errors.filter((e: TODO_ANY) => e.duplicate)
              .length;
            if (hidden > 0) {
              console.log(
                `${hidden} duplicate ${
            ...
            const hidden = event.warnings.filter((e: TODO_ANY) => e.duplicate)
              .length;
            if (hidden > 0) {
              console.log(
                `${hidden} duplicate ${
