...
export async function ejectCommand() {
  const prompt = new MultiSelect({
    ...
    name: 'files',
    message:
      'Pick files to copy out! **Note: use <Space> to pick files, <i> to invert selection**',
    choices: [
      {
      ...
      {
        name: '[sampleUID].svelte',
        value: 'src/routes/[sampleUID].svelte',
        hint: `an example detail page that pings ssg's default data route`
      },
      {
        name: 'template.html',
        value: 'src/template.html',
        hint: `sapper's template.html`
      },
      {
      ...
      { name: 'server.js', value: 'src/server.js', hint: `sapper's server.js` },
      {
        ...
        value: 'src/service-worker.js',
        hint: `sapper's service-worker.js`
      },
      {
        name: 'netlify.toml',
        value: 'netlify.toml',
        hint: `Netlify deploy config file for ssg`
      },
      {
        name: 'error.svelte',
        value: 'src/routes/_error.svelte',
        hint: `(unused) error.svelte`
      },
      {
        name: 'layout.svelte',
        value: 'src/routes/_layout.svelte',
        hint: `(unused) layout.svelte`
      }
    ],
    result(names: any) {
...
async function eject([_sourceFile, destinationPath]: string[]) {
  ...
  if (fs.existsSync(sourceFile)) {
    try {
      ...
      console.log(
        `${chalk.green('copied')} ${chalk.yellow(sourceFile)} to ${chalk.yellow(
          destinationPath
        )}`
      );
    } catch (err) {
